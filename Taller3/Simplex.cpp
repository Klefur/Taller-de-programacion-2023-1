...
Simplex::Simplex(string fileName) {
    ...
    for (int i = 1; i <= m; i++) {
        if (a[i][0] < 0.0 || (int)a[i].size() != n + 1) {
            cout << "Error en la cantidad de variables o constante menor que 0:"
                 << i << endl;
            exit(1);
        }
    }

    // EL metodo requiere una fila adicional al final de la matriz para calculos
    // intermedios
    vector<float> row;
    for (int i = 0; i <= n; i++) {
        row.push_back(0.0);
...
/*
    retorna el estado de la solucion: 0 si se encontro una solucion, 1 si no hay
   solucion, 2 si hay infinitas soluciones Ejecuta el algorithmo  simplex
*/
int Simplex::simplx() {
    ...
    if (m2 + m3) {
        ...
        for (;;) {
            maxValue(m + 1, l1, nl1, 0, &kp,
                     &bmax);  // buscando la variable con mayor coeficiente en
                              // la funcion Z
            if (bmax <= EPS &&
                a[m + 1][0] <
                    -EPS) {  // no hay solucion factible (Z auxiliar < 0)
                return -1;   // icase -1
            } else if (bmax <= EPS &&
                       a[m + 1][0] <=
                           EPS) {  // solucion factible (Z auxiliar = 0)
                for (ip = m1 + m2; ip < m; ip++) {
                    if (iposv[ip] == ip + n) {  // se busca la variable de
                ...
                for (i = m1; i < m1 + m2;
                     i++) {  // se cambia el signo de la restriccion de
                             // variables aun presentes
                    if (l3[i - m1] == 1) {
                        for (k = 0; k < n + 1; k++) {
                            a[i + 1][k] = -a[i + 1][k];
                        }
...
/*
   Simp1 en Numerical Recipes: Determina el maximo valor de una fila de la
   matriz a
*/
void Simplex::maxValue(int mm,
                       vector<int> ll,
                       int nll,
                       int iabf,
                       int* kp,
...
void Simplex::insertConstraint(float b, int var, int type) {
    ...
    switch (type) {
        case 1:
            ...
            break;
        case 2:
            this->initialA.insert(this->initialA.begin() + m1 + m2 + 1,
                                  constraint);
            m2++;
            break;
        case 3:
...
void Simplex::printSolution() {
    if (isSolved) {
        ...
        if (icase == 1) {
            ...
            for (int i = 0; i < n; i++) {
                if (izrov[i] < nm1m2) {
                    txt[izrov[i]].insert(txt[izrov[i]].begin(),
                                         10 - txt[izrov[i]].length(), ' ');
                    cout << txt[izrov[i]];
                }
            }
